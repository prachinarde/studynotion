...
const VideoDetails = () => {
  ...
  useEffect(() => {
    ;(async () => {
      ...
      if (filteredData.length === 0 || !filteredData[0]?.subSection) return
  
      const filteredVideoData = filteredData[0].subSection.filter((data) => data._id === subSectionId)
      if (filteredVideoData.length === 0) return
          ...
          {videoEnded && (
            <div style = {{backgroundImage:  "linear-gradient(to top, rgb(0, 0, 0), rgba(0,0,0,0.7), rgba(0,0,0,0.5), rgba(0,0,0,0.1)", }}
              ...
              {!completedLectures.includes(subSectionId) && (
                <IconBtn disabled={loading} onClickHandler={() => handleLectureCompletion()}  text={!loading ? "Mark As Completed" : "Loading..."}  customClasses="text-xl max-w-max px-4 mx-auto" />
              )}
